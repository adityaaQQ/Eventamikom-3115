...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            ...
            <h3 class="text-2xl font-black">Rp {{ number_format($total_pendapatan, 0, ',', '.') }}</h3>
        </div>
        <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            ...
            <h3 class="text-2xl font-black">{{ $total_tiket }}</h3>
        </div>
        <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            ...
            <h3 class="text-2xl font-black">{{ $total_event }} Event</h3>
        </div>
        <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            ...
            <h3 class="text-2xl font-black">{{ $total_pending }} Pesanan</h3>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-8 border-b flex justify-between items-center">
            ...
            <a href="{{ url('/adminlaporan') }}" class="text-indigo-600 font-bold hover:underline">Lihat Semua</a>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y border-t">
                    ...
                    <tr class="hover:bg-slate-50 transition">
                        <td class="px-8 py-6">
                            <p class="font-bold uppercase tracking-wide text-sm">{{ $trx->pembeli ?? '-' }}</p>
                        ...
                        <td class="px-8 py-6">
                            ...
                            @else
                                <span class="px-3 py-1 bg-slate-100 text-slate-600 rounded-lg text-xs font-bold uppercase">{{ $trx->status }}</span>
                            @endif
                        </td>
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="px-8 py-10 text-center text-slate-400 font-bold">Belum ada transaksi.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
